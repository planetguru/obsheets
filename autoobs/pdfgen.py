...
def _session_cover(d, y, sess_num, meet_name, club, venue, course, date_label, ev_range, pool_len):
    ...
    meta = "  ·  ".join(p for p in (course_lbl, venue, ev_range) if p)
...
def add_session(d, sess_num, sess_data, meet_name, club, venue, course, date_label):
    ...
    # swimmers arrive pre-sorted (slowest first, NT top) from the parser
    first = True
    for ev in events:
...
# ── ADAPTER: render a parsed Meet (autoobs.parse.Meet) to a single PDF ─────────

def _event_to_dict(ev):
    return {
        'eventNum': ev.number, 'gender': ev.gender, 'distance': ev.distance,
        'stroke': ev.stroke,
        'swimmers': [
            {'firstname': s.first, 'surname': s.surname, 'age': s.age,
             'entryTime': s.entry_time}
            for s in ev.swimmers
        ],
    }

def meet_to_pdf(meet) -> bytes:
    """Render every session of a parsed Meet into one PDF (each session a fresh page)."""
    d = Doc()
    club = "Bridgwater ASC"
    for s in meet.sessions:
        sess_data = {'events': {ev.number: _event_to_dict(ev) for ev in s.events}}
